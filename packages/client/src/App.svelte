<script lang="ts">
  import GardenView from "./ui/components/GardenView.svelte";
  ...
  function handleMovePlantInBed(bedId: string, plantId: string, newX: number, newY: number) {
  ...
  function handleMovePlantToDifferentBed(
    ...
    if (sourceBed && targetBed) {
      sourceBed.plantPlacements = sourceBed.plantPlacements.filter((p: PlantPlacement) => p.id !== plant.id);
      ...
      console.error('[App.svelte] Source or target bed not found for movePlantToDifferentBed.');
    }
  }
</script>
