...
async function main() {
	...
	if (args.length !== 2) {
		console.error(
			'Usage: ts-node scripts/slice-thumbnails.ts <input_folder> <output_folder>',
		)
		process.exit(1)
	}
			...
			try {
				...
				for (let i = 0; i < 9; i++) {
					...
					if (!(await fileExists(tempRawTilePath))) {
						console.warn(
							`Tile ${tempRawTilePath} not found after cropping. This might happen if the input image was not a 3x3 grid. Skipping.`,
						)
						continue
					}
